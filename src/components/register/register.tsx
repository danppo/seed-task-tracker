...
const Register = () => {
  ...
  const onSubmit = () => {
    ...
    .catch((err) => {
      if (err.response) {
        setErrorMessage(err.response.data.message ? err.response.data.message : "There has been an problem Please try again")
        setErrorStatus(true)
        ...
        {registerSuccess && cookieAuth &&
          <>
            <Alert severity="success">
              <AlertTitle>
                {serverMessage.length > 0 ? serverMessage : "Registration Successful"}
              </AlertTitle>
